...
export class AppleScriptDriver implements Driver {
  ...
  async #createGeminiSession(
    ...
    const filled = (await this.#eval(match, project.url, `
    ...
    const clicked = (await this.#eval(match, project.url, `
    ...
    if (!clicked.ok) throw new AichatctlError("Gemini 'Send message' button not found (calibration).");
    const norm = (u: string): string => u.replace(/\/+$/, "");
    let url = startUrl;
    for (let i = 0; i < 24; i++) {
      await sleep(500);
      const r = (await this.#eval(match, project.url, `return JSON.stringify({url:location.href});`)) as {
        url: string;
      };
      url = r.url;
